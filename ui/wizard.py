...
class WkWizard(QWizard):
    ...
    def _load(self, button):
        ...
        self.project, self.road = record.project, record.road; self.travelled_width_m=record.road.travelled_width_m; self.left=record.road.left; self.right=record.road.right; self.elements=record.travelled_elements
        self.project_page.load_details(record.project); self.travelled_page.load_composition(record.road.travelled_width_m,record.travelled_elements); self.left_page.load_side(record.road.left); self.right_page.load_side(record.road.right); self.limits_page.load_road(record.road); self.restart()
