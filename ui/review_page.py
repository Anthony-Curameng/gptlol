...
class ReviewPage(QWizardPage):
    ...
    def initializePage(self):
        result=calculate_wk(self.wizard().road); self.wizard().result=result; self.summary.setText(f"Travelled way: {result.travelled_width_m:.2f} m   •   Unrestricted Wk: {result.unrestricted_wk_m:.2f} m   •   Proposed final Wk: {result.final_wk_m:.2f} m"); self.view.draw_geometry(self.wizard().road,result,self.wizard().elements)
