    ...
    def draw_geometry(self, road: RoadInput, result: WkResult, elements: list[TravelledElement]):
        ...
        if result.lanes:
            ...
            for lane in result.lanes:
                lane_pixels=lane.lane_width_m*scale
                scene.addRect(lane_x,y,lane_pixels,height,QPen(QColor("#dbe4ea"),1),QBrush(QColor("#465866")))
                lane_label=scene.addSimpleText(f"{lane.name}\n{lane.lane_width_m:.2f} m"); lane_label.setBrush(QColor("white")); lane_label.setPos(lane_x+max(2,lane_pixels/2-lane_label.boundingRect().width()/2),y+28); lane_x+=lane_pixels
        ...
        if result.lanes:
            item=scene.addSimpleText("Lanes shown left → right; outside allowances belong to the first and last lanes."); item.setPos(left_edge,y-30)
