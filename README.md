...
In element-based mode, use one row per distinct lane width (or a quantity for repeated, equal lanes). Rows can be added and removed, so a 3.20 m lane, 3.50 m lane, and 4.00 m turn lane are retained and reported separately rather than averaged.
...
### Per-lane Wk convention

When an element schedule is supplied, the engine expands quantities into lanes ordered **left to right** and returns a `LaneResult` for every included lane. Each result records the lane's actual width, assigned outside allowances, lane Wk, and its interval within the overall physical Wk zone.

Outside allowance belongs only to the outside lanes: the left allowance is assigned to the first lane and the right allowance to the last lane. Interior lanes retain their entered width. If a kerb-to-kerb or sealed-width cap reduces the available outside allowance, the reduction is shared pro-rata between the requested left and right allowances. Therefore, the sum of all lane Wk values always equals the physically calculated Wk. The authority minimum or override remains a separate final rule step and is not silently redistributed across lane geometry.

This allocation is explicit rather than an assumed engineering standard. If the governing authority requires a different convention—for example, a full independent allowance around every lane—it should be added as a named rule profile and tested rather than changing the calculation invisibly.
